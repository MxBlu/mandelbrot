/*
   ...
   Generates an image based on the number of iteration of the Mandelbrot
   equation required to escape the bounds of circle with radius 2
   on the complex plane

   Colour is defined in pixelColour.c

   parser.h contains a necessary struct for the input of the triordinates
   and must be included with this file

   Image size is defined in bmpDetails.h as SIZE

   complex.c contains complex functions necessary to calulate the
   Mandelbrot equation and must be included with this file.
...
int escapeSteps (double x, double y) {
   ...
   while ( distC (z) <= 2 && stepCount < STEP_LIMIT ) {
      z = addC (multiC (z, z), c);
      stepCount++;
   }
...
void getMandelbrot (MandelbrotRequest inRequest, unsigned char * byteArray, int arraySize) {
   ...
   double pixelSize = exp2(-inRequest.z);

   double xStart = inRequest.x - ( pixelSize * ((double) SIZE / (double) 2) )
                     + (double) pixelSize / (double) 2;

   double xEnd = inRequest.x + ( pixelSize * ((double) SIZE / (double) 2) )
                     - (double) pixelSize / (double) 4;

   double yStart = inRequest.y - ( pixelSize * ((double) SIZE / (double) 2) )
                     + (double) pixelSize / (double) 2;

   double yEnd = inRequest.y + ( pixelSize * ((double) SIZE / (double) 2) )
                     - (double) pixelSize / (double) 4;


   double xCounter = xStart;
   double yCounter = yStart;
   ...
   while (yCounter < yEnd) {
      while (xCounter < xEnd) {
         ...
         stepsToEscape = escapeSteps(xCounter, yCounter);

         assert(arrayCounter < arraySize);
         byteArray[arrayCounter] = stepsToBlue(stepsToEscape);
         arrayCounter++;

         assert(arrayCounter < arraySize);
         byteArray[arrayCounter] = stepsToGreen(stepsToEscape);
         arrayCounter++;

         assert(arrayCounter < arraySize);
         byteArray[arrayCounter] = stepsToRed(stepsToEscape);
         arrayCounter++;
